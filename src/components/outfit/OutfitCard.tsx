      ...
      <CardContent className="space-y-4 flex-grow">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
          {outfit.items.map((item, index) => {
            const imageSrc = item.inputIndex >= 0 && item.inputIndex < uploadedItemPreviews.length 
              ? uploadedItemPreviews[item.inputIndex] 
              : `https://placehold.co/100x100.png`; // Fallback to placeholder
            ...
            return (
              <div key={`${item.name}-${index}`} className="border border-border p-3 rounded-md bg-background/50 flex flex-col items-center text-center shadow-sm">
                <Image
                  ...
                  data-ai-hint={getItemKeywords(item.name)}
                />
